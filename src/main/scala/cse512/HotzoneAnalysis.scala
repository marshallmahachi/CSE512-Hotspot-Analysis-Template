...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    //we now need to return the rectangle and the count of points in the rectangle ..
    //so this is just a group_by rectangle then sum.. :)
    val zone_counts = spark.sql("select rectangle, count(point) from joinResult group by rectangle order by rectangle")

    //zone_counts.show()

    return zone_counts.toDF() // think I should return this one
  }
